...
cp "$REPO_ROOT/master/task_map.yaml"       "$CASE_DIR/adapted_task_map.yaml"
...
echo "  4. Edit $CASE_DIR/adapted_task_map.yaml — replace {PLACEHOLDER} with real values, tune questions"
echo "  5. Run: python scripts/adapt_case.py --case cases/$CASE_ID"
echo "  6. Run LLM on adapted_indicators.xlsx → save results to $CASE_DIR/validation/run_output.json"
echo "  7. Fill in $CASE_DIR/human_grades.yaml"
echo "  8. Run: python scripts/validate_case.py --case cases/$CASE_ID"
echo "  9. Annotate $CASE_DIR/validation/diff_report.yaml (error_types, fix_applied, etc.)"
